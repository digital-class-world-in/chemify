...
export default function BatchManagementPage() {
  ...
  const filteredBatches = useMemo(() => {
    ...
    return list.filter(b => 
      ...
      b.className?.toLowerCase().includes(lowerSearch)
    )
    ...
    <main className="flex-1 p-8 space-y-10 animate-in fade-in duration-500 w-full overflow-x-hidden font-public-sans text-black text-[14px]">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 px-2">
        <div>
          ...
          <p className="text-sm text-zinc-400 font-medium mt-1">Schedule and manage institutional academic nodes</p>
        </div>
        {!isPortal && (
      ...
      <Card className="border border-zinc-100 shadow-sm rounded-[32px] overflow-hidden bg-white">
        <div className="p-8 border-b border-zinc-50 flex items-center justify-between">
          <div className="relative group w-full md:w-80">
            ...
            <Input placeholder="Search Registry..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-11 h-11 bg-zinc-50 border-none rounded-xl text-sm font-bold shadow-inner transition-none focus-visible:ring-primary" />
          </div>
        </div>
          ...
          <Table className="min-w-[1400px]">
            <TableHeader className="bg-zinc-50/50">
              <TableRow className="border-zinc-100">
                ...
                <TableHead className="text-[11px] font-black text-zinc-400 uppercase tracking-widest h-14">BATCH NAME</TableHead>
                <TableHead className="text-[11px] font-black text-zinc-400 uppercase tracking-widest h-14">CLASS / COURSE</TableHead>
                ...
                <TableRow key={row.id} className="border-zinc-50 hover:bg-zinc-50/30 transition-none group text-black font-public-sans">
                  ...
                  <TableCell className="py-6">
                    <div className="flex items-center gap-4">
                      <div className="w-10 h-10 rounded-xl bg-zinc-50 flex items-center justify-center text-[#1e3a8a] shadow-inner"><Layers className="w-5 h-5" /></div>
                      <span className="text-sm font-black text-zinc-800 uppercase font-headline tracking-tight">{row.batchName}</span>
                    </div>
                  </TableCell>
                  <TableCell>
                    <div className="flex flex-col">
                      <span className="text-sm font-bold text-zinc-700 uppercase">{row.className}</span>
                      <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest">{row.courseName}</span>
                    </div>
                  </TableCell>
                  ...
                  <TableCell className="text-center">
                    <Badge className={cn(
                      ...
                      row.status === 'Ongoing' ? "bg-emerald-50 text-emerald-600" : "bg-blue-50 text-blue-600"
                    )}>{row.status}</Badge>
                  </TableCell>
                  {!isPortal && (
          ...
          <ScrollArea className="max-h-[80vh]">
            <form onSubmit={handleSaveBatch} className="p-10 space-y-12">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-10 gap-y-8">
                <div className="md:col-span-2 space-y-1.5"><Label className="text-[13px] font-bold text-zinc-400 uppercase tracking-widest ml-1">Batch Identifier *</Label><Input name="batchName" defaultValue={selectedBatch?.batchName} required className="rounded-2xl border-zinc-100 bg-zinc-50/50 h-12 font-black text-zinc-800 text-base shadow-inner focus-visible:ring-primary" /></div>
                <div className="space-y-1.5"><Label className="text-[11px] font-bold text-zinc-400 uppercase tracking-widest">Academic Class</Label><Select name="className" defaultValue={selectedBatch?.className} required><SelectTrigger className="h-12 rounded-2xl border-zinc-100 font-bold bg-white"><SelectValue placeholder="Select" /></SelectTrigger><SelectContent className="rounded-2xl shadow-xl">{(dropdownData['class'] || []).map(opt => <SelectItem key={opt.id} value={opt.value} className="font-bold text-[14px] uppercase">{opt.value}</SelectItem>)}</SelectContent></Select></div>
                <div className="space-y-1.5"><Label className="text-[11px] font-bold text-zinc-400 uppercase tracking-widest">Course Catalog</Label><Select name="courseName" defaultValue={selectedBatch?.courseName} required><SelectTrigger className="h-12 rounded-2xl border-zinc-100 font-bold bg-white"><SelectValue placeholder="Select" /></SelectTrigger><SelectContent className="rounded-2xl shadow-xl">{(dropdownData['course'] || []).map(opt => <SelectItem key={opt.id} value={opt.value} className="font-bold text-[14px] uppercase">{opt.value}</SelectItem>)}</SelectContent></Select></div>
                <div className="space-y-1.5"><Label className="text-[11px] font-bold text-zinc-400 uppercase tracking-widest">Assigned Faculty</Label><Select name="faculty" defaultValue={selectedBatch?.faculty} required><SelectTrigger className="h-12 rounded-2xl border-zinc-100 font-bold bg-white"><SelectValue placeholder="Select" /></SelectTrigger><SelectContent className="rounded-2xl shadow-xl">{staffList.map(s => <SelectItem key={s.id} value={`${s.firstName} ${s.lastName}`} className="font-bold text-[14px] uppercase">{s.firstName} {s.lastName}</SelectItem>)}</SelectContent></Select></div>
                ...
                <div className="space-y-1.5"><Label className="text-[11px] font-bold text-zinc-400 uppercase tracking-widest">Lifecycle Status</Label><Select name="status" defaultValue={selectedBatch?.status || "Upcoming"}>
                  <SelectTrigger className="h-12 rounded-xl border-zinc-200 font-bold bg-white"><SelectValue /></SelectTrigger>
                  <SelectContent className="rounded-2xl shadow-xl"><SelectItem value="Upcoming" className="font-bold">UPCOMING</SelectItem><SelectItem value="Ongoing" className="font-bold">ONGOING</SelectItem><SelectItem value="Completed" className="font-bold">COMPLETED</SelectItem></SelectContent>
                </Select></div>
              </div>
              <div className="pt-8 border-t border-zinc-100"><Button type="submit" className="w-full h-16 bg-zinc-900 hover:bg-black text-white rounded-3xl font-black uppercase text-xs tracking-[0.2em] shadow-xl border-none active:scale-95 transition-all flex items-center justify-center gap-3"><Save className="w-5 h-5" /> {selectedBatch ? 'Synchronize All Updates' : 'Commit Batch Registry'}</Button></div>
            </form>
          </ScrollArea>
        </DialogContent>
      </Dialog>
