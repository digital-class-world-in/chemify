...
const DEFAULT_BLOGS = [
  {
    ...
    description: "Discover the most high-paying and stable career paths...",
  ...
  },
  {
    ...
    description: "Moving beyond degrees: practical skills...",
    author: "Dr. R. Sharma",
    date: "Feb 12, 2024",
    readTime: "8 min",
  }
  ...
  return (
    <div className="min-h-screen bg-white overflow-x-hidden selection:bg-primary/20" style={{ fontFamily: settings.styling?.fontFamily || 'Poppins' }}>
          ...
          <div className="flex items-center gap-4">
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-56 p-0 border-zinc-100 shadow-2xl rounded-2xl overflow-hidden mt-4 bg-white">
                ...
                <DropdownMenuItem asChild>
                  <Link href="/branch/login" className="text-sm font-bold text-zinc-700 uppercase tracking-tight">
                    Branch Login
                  </Link>
                </DropdownMenuItem>
              </DropdownMenuContent>
      ...
      {settings.visibility?.hero !== false && (
        ...
        >
          {/* Your hero content – keep as is or customize */}
          ...
          <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center w-full relative z-10">
            <motion.div {...fadeIn} className="space-y-6 lg:space-y-8">
              <h1 className="font-black leading-[1.1] tracking-tight text-zinc-900" style={heroTitleStyle}>
                {settings.hero?.title || "Welcome to Excellence in Education"}
              </h1>
              <p className="text-base lg:text-lg font-medium leading-relaxed max-w-xl text-zinc-600" style={heroDescStyle}>
                {settings.hero?.description || "Empowering students with knowledge, skills, and values for a bright future."}
              </p>
              <div className="flex flex-wrap gap-4 pt-2 lg:pt-4">
                ...
                >
                  {settings.hero?.buttonText || "Explore Courses"}
        ...
        <section id="courses" style={sectionPadding} className="bg-zinc-100 -mt-16 lg:-mt-24">
          <div className="max-w-7xl mx-auto px-5 sm:px-6 space-y-6 lg:space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 lg:gap-7">
              {courses
                .filter(c => c.showOnHomepage && c.status !== 'Draft')
                .map((course, i) => (
        ...
        <section className="bg-white border-y border-zinc-100 py-16 overflow-hidden">
          <div className="max-w-7xl mx-auto px-5 sm:px-6 space-y-6 lg:space-y-8">
            <div className="text-center max-w-3xl mx-auto">
              ...
              >
                {settings.milestoneHeading?.text || "Our Impact & Reach"}
              </h2>
            </div>
          ...
          <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24 items-start">
            <motion.div {...fadeIn} className="relative group lg:sticky lg:top-32">
              <div className="aspect-[4/5] rounded-2xl overflow-hidden shadow-2xl border-8 border-zinc-100">
                <img
                  src={settings.about?.imageUrl || "https://picsum.photos/seed/about-main/800/1000"}
            ...
            <motion.div {...fadeIn} transition={{ delay: 0.3 }} className="space-y-8">
              <div className="space-y-4">
                <Badge className="bg-primary/10 text-primary border-none uppercase font-black text-[10px] px-4 tracking-widest rounded-full">
                  {settings.about?.badge || 'About Us'}
                </Badge>
                <h2 className="text-4xl lg:text-6xl font-black text-zinc-900 uppercase tracking-tight leading-[0.9]">
                  {settings.about?.title || 'Building Futures Since 2005'}
                </h2>
              </div>
              <div className="space-y-6 text-zinc-600 text-lg leading-relaxed">
                <p>{settings.about?.description1 || "We are committed to providing quality education..."}</p>
                <p>{settings.about?.description2 || "Our mission is to empower students..."}</p>
              </div>
            </motion.div>
          </div>
        </section>
                        ...
                        <Card className="bg-white rounded-2xl border border-zinc-100 shadow-sm hover:shadow-xl transition-all duration-300 h-full flex flex-col">
                          ...
                          <div className="p-6 md:p-8 flex-1 flex flex-col">
                            ...
                            <div className="flex items-center justify-between text-xs text-zinc-500 font-medium">
                              ...
                              <span className="flex items-center gap-1.5">
                                <Clock className="w-4 h-4" /> {blog.readTime || '5 min'}
                              </span>
                            </div>
                          </div>
                        </Card>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
              <motion.div {...fadeIn} className="space-y-10">
                <div>
                  ...
                  <h2 className="text-4xl lg:text-5xl font-black text-zinc-900">Let's Connect</h2>
                ...
                <div className="space-y-6">
                  <ContactCard icon={<Phone className="w-6 h-6" />} label="Phone" value={publicContact.phone} />
                  <ContactCard icon={<Mail className="w-6 h-6" />} label="Email" value={publicContact.email} />
                  <ContactCard icon={<MapPin className="w-6 h-6" />} label="Address" value={publicContact.address} />
                </div>
              </motion.div>
              ...
              <motion.div {...fadeIn} transition={{ delay: 0.2 }} className="bg-white rounded-3xl shadow-2xl p-8 lg:p-12 relative overflow-hidden">
                {isSubmitted ? (
                  <div className="py-16 text-center space-y-6">
                    ...
                    <p className="text-zinc-600">Your inquiry has been received. We'll get back to you soon.</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-16">
            <div className="space-y-6">
              {profile?.logoUrl ? (
                ...
                <h3 className="text-2xl font-black">Nirvana Education</h3>
              )}
...
<SocialBtn href={settings.social.youtube} icon={Youtube} />
            ...
            <div>
              ...
              <ul className="space-y-3 text-zinc-400">
                ...
                <li><Link href="/infrastructure" className="hover:text-white transition">Facilities</Link></li>
            ...
            <div>
              ...
              <ul className="space-y-4 text-zinc-400">
                <li className="flex items-start gap-3">
                  ...
                  <span>{publicContact.address || "Ahmedabad, Gujarat"}</span>
                </li>
          ...
          <div className="pt-10 border-t border-zinc-800 text-center text-zinc-500 text-sm">
            © {format(new Date(), "yyyy")} {profile?.instituteName || "Nirvana Education Commune"} • All rights reserved.
          </div>
        </div>
      </footer>
      ...
      <Dialog open={isApplyModalOpen} onOpenChange={setIsApplyModalOpen}>
        <DialogContent className="max-w-lg rounded-3xl p-0 overflow-hidden border-none shadow-2xl">
          <div className="bg-gradient-to-br from-primary to-purple-600 p-10 text-white relative">
            ...
            <DialogTitle className="text-3xl font-black mb-3">Get Started Today</DialogTitle>
            <p className="text-white/90 mb-8">Fill this form and our counselor will reach out within 24 hours.</p>
            ...
            {isSubmitted ? (
              <div className="bg-white/10 backdrop-blur-lg rounded-2xl p-10 text-center">
                ...
                <p className="text-white/90">Your inquiry has been submitted successfully.</p>
                <Button
              ...
              <form onSubmit={handleFormSubmit} className="space-y-6">
                ...
                <div>
                  <Label className="text-white/90 mb-2 block">Interested Course</Label>
...
function VarietySlider({ slides, height }: { slides: any[]; height: number }) {
  ...
  const images = slides?.length > 0 ? slides : [
    { imageUrl: "https://picsum.photos/seed/education1/800/1000" },
    { imageUrl: "https://picsum.photos/seed/education2/800/1000" },
  ]
    ...
    <Link href={`/course/${course.id}`} className="block h-full group">
      <Card className="border-2 border-zinc-200 rounded-2xl overflow-hidden group-hover:shadow-2xl group-hover:-translate-y-2 transition-all duration-300 flex flex-col h-full">
        <div className="h-56 relative overflow-hidden">
          <img
            src={course.image || `https://picsum.photos/seed/${course.name}/600/400`}
            className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110"
            alt={course.name}
          />
          {course.type && (
        ...
        <CardContent className="p-6 flex-1 flex flex-col">
          ...
          <p className="text-zinc-600 text-sm mb-4 flex-1 line-clamp-3">{course.description}</p>
          <div className="flex items-center gap-3 mt-auto">
            <span className="text-2xl font-black text-primary">
              ₹{Number(course.sellingPrice || course.price).toLocaleString()}
            </span>
            {discount > 0 && (
...
function TestimonialCard({ t }: { t: any }) {
  return (
    <div className="min-w-[380px] md:min-w-[450px] p-8 rounded-3xl bg-white border border-zinc-100 shadow-lg">
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-zinc-500">{t.role || "Student"}</p>
        </div>
      </div>
      <Quote className="w-12 h-12 text-primary/20 mb-4" />
      <p className="text-zinc-700 leading-relaxed line-clamp-4">"{t.message}"</p>
    </div>
  )
}
  ...
  return (
    <div className="flex items-center gap-5 p-6 rounded-2xl bg-white border border-zinc-100 shadow-sm hover:shadow-md transition-all">
      ...
      <div>
        ...
        <p className="text-lg font-semibold text-zinc-900">{value || "N/A"}</p>
      </div>
    </div>
  )
}
