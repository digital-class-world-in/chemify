...
export default function StudentHolidaysPage() {
  ...
  useEffect(() => {
    ...
    setIsLoading(true)
    const dbRef = ref(database, `Institutes/${resolvedId}/holidays`)
  ...
  if (isLoading || idLoading) return <div className="p-20 text-center font-black text-zinc-300 uppercase tracking-[0.2em] animate-pulse">Establishing Academic Calendar...</div>
  ...
  return (
    <main className="flex-1 p-8 space-y-10 animate-in fade-in duration-700 max-w-7xl mx-auto pb-32 font-public-sans text-black">
      ...
      <div className="flex flex-col md:flex-row justify-between items-end gap-6">
        <div>
          <h2 className="text-3xl font-black text-zinc-800 uppercase tracking-tight leading-none">Holiday Registry</h2>
          <p className="text-sm text-zinc-400 font-medium uppercase tracking-widest mt-1">Plan your schedule with official institutional breaks</p>
        </div>
        <div className="flex items-center gap-3 bg-white p-1.5 rounded-2xl shadow-sm border border-zinc-100">
          <Button variant="ghost" size="icon" onClick={() => setCurrentMonth(subMonths(currentMonth, 1))} className="h-10 w-10 rounded-xl transition-all hover:bg-zinc-50"><ChevronLeft className="w-5 h-5 text-zinc-400" /></Button>
          <div className="px-6 text-[11px] font-black uppercase tracking-[0.25em] text-zinc-700 min-w-[200px] text-center">
            {format(currentMonth, "MMMM yyyy")}
          </div>
          <Button variant="ghost" size="icon" onClick={() => setCurrentMonth(addMonths(currentMonth, 1))} className="h-10 w-10 rounded-xl transition-all hover:bg-zinc-50"><ChevronRight className="w-5 h-5 text-zinc-400" /></Button>
        </div>
      </div>

      <Card className="border-none shadow-2xl rounded-[48px] overflow-hidden bg-white border border-zinc-200">
          ...
          {calendarDays.map((day, idx) => {
            ...
            return (
              <div key={idx} className={cn(
                "min-h-[160px] p-4 border-r border-b border-zinc-200 relative group transition-all duration-500",
          ...
          {viewingHoliday && (
            <>
              <div className={cn(
                ...
                <DialogClose className="absolute right-8 top-8 p-2 rounded-full hover:bg-white/10 text-white/40 border-none transition-colors outline-none cursor-pointer"><X className="h-6 w-6" /></DialogClose>
                <div className="space-y-6">
                  <Badge className="bg-white/20 text-white border-none text-[10px] font-black uppercase tracking-widest px-4 py-1">Institutional Schedule</Badge>
                  ...
                  <div className="flex items-center gap-4 text-xs font-black text-white/60 uppercase tracking-widest">
                    <span className="flex items-center gap-2"><PartyPopper className="w-4 h-4" /> {viewingHoliday.entryType || 'General'}</span>
                  </div>
                </div>
              </div>
              <div className="p-12 space-y-10">
                <div className="grid grid-cols-2 gap-8">
                  <div className="space-y-1.5">
                    <p className="text-[10px] font-black text-zinc-300 uppercase tracking-widest">Schedule Window</p>
                  ...
                  <div className="space-y-1.5">
                    <p className="text-[10px] font-black text-zinc-300 uppercase tracking-widest">Campus Node</p>
                    <div className="flex items-center gap-3 text-sm font-black text-rose-500">
                      <ShieldCheck className="w-4 h-4 text-primary" /> Verified Break
                    </div>
                  </div>
                </div>

                <div className="space-y-4">
                  <h5 className="text-[10px] font-black text-zinc-400 uppercase tracking-[0.2em] border-b border-zinc-50 pb-2">Institutional Remarks</h5>
                ...
                <div className="pt-4 text-center">
                  <p className="text-2xl font-black text-zinc-800 uppercase tracking-tight leading-none">Happy Holidays! 🎉</p>
                  <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest mt-2">Authorized Publication Node</p>
                </div>
              </div>
            </>
          )}
