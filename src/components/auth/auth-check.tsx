...
export function AuthCheck({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    // 1. Handle Super Admin Portal Auth
    if (isSuperPath) {
    ...
    // 5. Handle Admin Portal Auth (Firebase Auth)
    // Root path is public if the institute setting allows it, handled in page.tsx
    ...
    // 6. Access Control for Deleted Nodes
          ...
          get(institutesRef).then((snap) => {
            ...
            Object.keys(data).forEach(instId => {
              ...
              Object.values(adms).forEach((a: any) => {
                if (a.email?.toLowerCase() === user.email?.toLowerCase()) isFound = true
              })
            });
  ...
  function isSpecialPath(p: string) {
    return (p.startsWith("/student/") || p === "/student") || 
  ...
  const erpDomains = ['erp.digitalclassworld.in', 'localhost'];
  const isErpDomain = erpDomains.some(d => hostname === d || hostname.includes('webstation-'));
  const isPublicWebsite = pathname.startsWith("/sites/") || pathname.startsWith("/business/") || (!isErpDomain && pathname === "/")
  
  if (isUserLoading && !isSpecialPath(pathname) && !isPublicWebsite && !PUBLIC_PATHS.includes(pathname)) {
    return (
